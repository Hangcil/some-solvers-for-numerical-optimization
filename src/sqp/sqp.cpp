...
sqp_o mini_optim::sqp(const sqp_i &input)
{
    ...
    unsigned num_nonlin = input.c(x).size(), num_nonlin_eq = input.ceq(x).size();
    vector<double> lambda(num_nonlin + num_nonlin_eq + input.A.size() + input.Aeq.size(), 0.0);
    unsigned iter = 0;
    while (true)
    {
        iter++;
        ...
        for (auto i = 0; i < num_nonlin; i++)
        {
            auto f = [&](vector<double> x) -> double
            { return input.c(x)[i]; };
            A.push_back(grad(f, x));
            b.push_back(input.c(x)[i]);
        }
        for (auto i = 0; i < num_nonlin_eq; i++)
        {
            auto f = [&](vector<double> x) -> double
            { return input.ceq(x)[i]; };
            Aeq.push_back(grad(f, x));
            beq.push_back(input.ceq(x)[i]);
        }
        qp_i in;
        in.H = B;
        in.c = g;
        ...
        double c = accumulate(beq.begin(), beq.end(), 0.0);
        double mu = (dot(g, p) + 0.5 * dot(p, multiply(B, p))) / (2.0 * c), alpha = 1.0;
        auto phi = [&](vector<double> x) -> double
        {
            auto temp1 = add(multiply(input.A, x), multiply(-1.0, input.b)), temp2 = input.c(x);
            double c = accumulate(temp1.begin(), temp1.end(), 0.0);
            c = accumulate(temp2.begin(), temp2.end(), c);
            return input.f(x) + mu * c;
        };
        unsigned exit = 0;
        while (phi(add(x, multiply(alpha, p))) > phi(x) + eta * alpha * (dot(g, p) - mu * c) && exit <= 20)
        {
            exit++;
            alpha *= tau;
        }
        ...
        if (dot(res, res) <= 0.0000000001 || iter >= input.max_iter)
        {
            sqp_o ret;
            ret.iter = iter;
            ret.success = true;
        ...
        for (auto i = 0; i < num_nonlin; i++)
        {
            auto f = [&](vector<double> x) -> double
            { return input.c(x)[i]; };
            y = add(y, multiply(-lambda[i + input.A.size()], add(grad(f, x), multiply(-1.0, grad(f, save)))));
        }
        ...
        auto rk = add(multiply(theta, y), multiply(1.0 - theta, multiply(B, s)));
        B = add(B, multiply(-1.0 / temp1, multiply(temp3, temp3)));
        B = add(B, multiply(1.0 / temp2, multiply(rk, rk)));
    }
}
*/
